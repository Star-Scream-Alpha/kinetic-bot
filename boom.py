...
def extract_features(window_data):
    ...
    prices = np.array(window_data['LTP'].values, dtype=np.float64)
    volumes = np.array(window_data['Volume'].values, dtype=np.float64)
...
class MLTradingBrain:
    def __init__(self, model1_data, model2_data, model3_data, feature_names):
        ...
        # FALLBACK: Use rule-based thresholds
        self.USE_FALLBACK = True  # Toggle this
        self.KINETIC_THRESHOLD = 37500
        self.DEGREE_THRESHOLD = 3.5
    ...
    def process_window(self, window_data):
        ...
        # FALLBACK MODE: Use rule-based logic
        if self.USE_FALLBACK:
            kinetic_score = features_dict['kinetic_score']
            hvg_degree = features_dict['hvg_mean_degree']
            price_trend = features_dict['price_trend']
            
            # Simulate model scores for display
            self.last_m1_score = min(kinetic_score / self.KINETIC_THRESHOLD, 1.0)
            self.last_m2_class = 1 if hvg_degree < self.DEGREE_THRESHOLD else 0
            self.last_m2_confidence = 0.85 if self.last_m2_class == 1 else 0.15
            
            # Rule-based decision (original logic)
            if kinetic_score > self.KINETIC_THRESHOLD:
                if hvg_degree < self.DEGREE_THRESHOLD:
                    # Inverse logic
                    direction = -1 if price_trend > 0 else 1
                    self.last_m3_direction = direction
                    self.last_m3_return = 45.0  # Expected return
                    return direction
            
            return 0
        
        # ML MODE (original code)
        try:
    ...
    for tick in sim:
        ...
        if tick['log']:
            ...
            <div style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 10px;">
                ...
                <p style="color: #38ef7d;"><strong>Win Rate:</strong> {win_rate:.1f}%</p>
                <p style="color: #667eea;"><strong>Total P&L:</strong> ₹{df_log['PnL'].sum():,.0f}</p>
            </div>
            """, unsafe_allow_html=True)
        else:
        ...
        if time_compression and last_tick_timestamp:
            market_time = (tick['ts'] - last_tick_timestamp).total_seconds()
            real_sleep = market_time / 60.0
            if real_sleep > 0:
